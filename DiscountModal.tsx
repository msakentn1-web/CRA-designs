...
export default function DiscountModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Secret code to open Command Center (Maestro Mode)
    if (trimmed === '22768061') {
      onOpenAdmin();
      onClose();
      setInputCode('');
    ...
    // Standard client discount coupon check
        ...
        <div className="flex items-center justify-between mb-6 border-b border-white/5 pb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-500">أدخل الرمز للحصول على تخفيض فوري لطلبك</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full text-center px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-[#0F52BA] focus:ring-1 focus:ring-[#0F52BA]/30 text-white text-xs outline-none transition-all font-mono tracking-widest uppercase"
              autoFocus
            />
          </div>
